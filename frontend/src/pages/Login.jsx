...
const ENTREPRENEUR = {
  ...
  badgePing: '#fbbf24',
  tag: 'FOR FOUNDERS',
  tagline: 'APPLICATIONS OPEN',
  headline: 'Your startup deserves',
  headlineBold: 'global capital.',
  sub: 'Submit your pitch once. Get discovered by 340+ vetted investors across 60+ countries. No gatekeepers, no cold emails, zero commission.',
  quote: '"The best startups don\'t chase investors — they build something investors can\'t ignore."',
  features: [
    { icon: '→', text: 'Complete your profile in under 20 minutes' },
    { icon: '→', text: 'Discovered by 340+ global investors automatically' },
    { icon: '→', text: 'Zero commission — every dollar raised is yours' },
  ],
  stats: [
...
const INVESTOR = {
  ...
  headlineBold: 'deal flow.',
  sub: 'Every pitch is audit-reviewed and compliance-checked. Access 500+ vetted opportunities. Deploy capital from $5K alongside institutional co-investors.',
  quote: '"Diversification without conviction is simply expensive ignorance."',
  features: [
    { icon: '→', text: '500+ audit-verified startup pitches — zero noise' },
    { icon: '→', text: 'Lead syndicates and earn 20% carried interest' },
    { icon: '→', text: 'Co-invest from $5K alongside institutional partners' },
  ],
  stats: [
    { v: '340+', l: 'Verified Investors' },
    { v: '60+', l: 'Countries' },
    { v: '20%', l: 'Carry Earned' },
  ],
      ...
      {!isInvestor ? (
        <div className="hidden lg:flex lg:w-[55%] relative z-10 flex-col justify-between px-12 py-8 xl:px-16">
          <div>
            ...
            <p className="text-white/80 text-sm leading-relaxed max-w-[340px] mb-4">{p.sub}</p>
        ...
        <div className="hidden lg:flex lg:w-[48%] relative z-10 flex-col justify-between px-12 py-8 xl:px-16">
          <div>
            ...
            <p className="text-white/80 text-sm leading-relaxed max-w-[320px] mb-4">{p.sub}</p>
      ...
      <div className={`w-full ${isInvestor ? 'lg:w-[52%]' : 'lg:w-[45%]'} relative z-10 flex items-center justify-center px-6 py-4`}>
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 mb-4">
            <div>
              <label className="block text-xs font-semibold mb-1" style={{ color: 'rgba(255,255,255,0.42)' }}>Email</label>
              <input
            ...
            <div>
              <label className="block text-xs font-semibold mb-1" style={{ color: 'rgba(255,255,255,0.42)' }}>Password</label>
          ...
          <div className="mb-3 rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <p className="text-[10px] font-black uppercase tracking-widest mb-2.5 text-center" style={{ color: 'rgba(255,255,255,0.35)' }}>
              ⚡ Demo Accounts — click to instant login
            </p>
            <div className="grid grid-cols-2 gap-2">
              {DEMO_ACCOUNTS.map(acc => (
                <button key={acc.email} onClick={() => loginAsDemo(acc)} disabled={loading}
                  className="flex items-center gap-2.5 py-2.5 px-3 rounded-xl transition-all disabled:opacity-40 active:scale-95 text-left"
                  style={{ background: 'rgba(255,255,255,0.05)', border: `1px solid ${acc.color}30` }}
                  onMouseEnter={e => { e.currentTarget.style.background = acc.color + '18'; e.currentTarget.style.border = `1px solid ${acc.color}70` }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.border = `1px solid ${acc.color}30` }}>
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black flex-shrink-0"
                    style={{ background: acc.color + '25', color: acc.color }}>
                    {acc.role[0]}
                  </div>
                  <div>
                    <div className="text-xs font-black" style={{ color: 'rgba(255,255,255,0.75)' }}>{acc.role}</div>
                    <div className="text-[9px]" style={{ color: 'rgba(255,255,255,0.28)' }}>{acc.email}</div>
                  </div>
                </button>
              ))}
            </div>
